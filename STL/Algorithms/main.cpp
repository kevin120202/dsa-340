...
void find_test() {
    ...
    auto loc1 = std::find(std::begin(players), players.end(), Person{ "Moe", 20 });
    if (loc1 != players.end())
...
void count_if_test() {
    ...
    num = std::count_if(std::begin(vec), std::end(vec), [](int x) {return x >= 100;});
    std::cout << num << " numbers are >= 100" << std::endl;
}
