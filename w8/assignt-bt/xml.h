...
public:
  ...
  std::string
      fulltext; // keep the full text of the opening tag, or the full plain text
  std::map<std::string, std::string> attrs;
  bool closed; // starts as false, used to know whether new items are children
               // or siblings

  xml_element() {}
  xml_element(xml_type type, const std::string &name,
              const std::string &fulltext,
              const std::map<std::string, std::string> &attrs)
      : type(type), name(name), fulltext(fulltext), attrs(attrs),
        closed(false) {}
};
...
int xml_handle_tag(std::string &tagstring, xml_tree_state &state,
                   bool verbose = false);
int xml_handle_plaintext(std::string &plaintext, xml_tree_state &state,
                         bool verbose = false);
int xml_handle_attributes(const std::string &input,
                          std::map<std::string, std::string> &attrs,
                          bool verbose = false);

int xml_add_node(const xml_element &element, xml_tree_state &state,
                 bool verbose = false);
int xml_close_tag(const std::string name, xml_tree_state &state,
                  bool verbose = false);
...
std::vector<XMLNODE *> xml_find_with_attr(XMLNODE *root,
